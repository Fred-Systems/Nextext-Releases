...
export default function ConversationScreen({ myUid, chatId: initialChatId, otherUid, contact, onBack, onOpenProfile, onOpenGroupInfo, onOpenChat, openSettings = false, showScrollDownSetting = true, scrollDownSize = 22, scrollDownPos = "center", animatedScrollEntry = false, recordingBarScale = 1, userDoc, emojiAnimations = true, emojiBigOn = true, onOpenAskAI, onOpenStatus, initialText = "" }) {
  ...
  // Real MIME of the generated bytes (from the Fish Audio worker response,
  // surfaced via the synthesized Blob's type). Used to pick the download
  // extension from actual bytes, never by renaming.
  const [genMime, setGenMime] = useState("");
  const [genSaving, setGenSaving] = useState(false);
  // Preview-composer playback state (custom Play/Pause + seek, replayable).
  ...
  // Revoke any stashed cloned/generated voice object URLs on unmount.
  const genUrlRef = useRef(null);
  const vcUrlRef = useRef(null);
  useEffect(() => { genUrlRef.current = genUrl; vcUrlRef.current = vcUrl; });
  useEffect(() => () => {
    try { if (genUrlRef.current) URL.revokeObjectURL(genUrlRef.current); } catch {}
    try { if (vcUrlRef.current) URL.revokeObjectURL(vcUrlRef.current); } catch {}
  }, []);
  ...
  // ── Chat drafts (composer input side, localStorage only — no Firebase) ──
  // Key format is EXACT (`nextext_draft_<chatId>`) — another component reads it.
  ...
  // On chat open/mount (and when chatId changes): flush the previous chat's
  // pending keystrokes, then load this chat's own draft into the composer.
  // Declared BEFORE the share-prefill effect so an explicit share prefill wins.
  useEffect(() => {
    const prev = prevChatRef.current;
    if (prev && prev !== chatId) {
      if (draftTimer.current) { clearTimeout(draftTimer.current); draftTimer.current = null; }
      try {
        if (input && !editingMsg) localStorage.setItem(`nextext_draft_${prev}`, input);
    ...
    let draft = null;
    try { draft = localStorage.getItem(`nextext_draft_${chatId}`); } catch { /* ignore */ }
    if (draft) {
      setInput(draft);
    } else if (!draftInitRef.current && initialText) {
  ...
  // Debounced (~500ms) draft persist on change. localStorage only — drafts are
  // never written to Firestore.
  useEffect(() => {
    if (!chatId || editingMsg || draftLoadedForRef.current !== chatId) return;
    if (draftTimer.current) clearTimeout(draftTimer.current);
    const key = `nextext_draft_${chatId}`;
    const val = input;
    draftTimer.current = setTimeout(() => {
      ...
      try {
        ...
        // Nudge the chat list so a visible row can refresh its Draft prefix.
        try { window.dispatchEvent(new Event("nextext-draft-change")); } catch { /* ignore */ }
      } catch { /* ignore */ }
    }, 500);
    return () => { if (draftTimer.current) { clearTimeout(draftTimer.current); draftTimer.current = null; } };
  }, [input, chatId, editingMsg]);

  // Unmount: flush any pending (<500ms old) keystrokes so nothing is lost.
  // (Chat switches are handled by the load effect above; this covers close.)
  useEffect(() => () => {
    if (draftTimer.current) { clearTimeout(draftTimer.current); draftTimer.current = null; }
    try {
      const cid = chatIdLiveRef.current;
      const v = inputLiveRef.current;
      if (cid && v) localStorage.setItem(`nextext_draft_${cid}`, v);
    } catch { /* ignore */ }
  }, []);

  const clearDraftForChat = (cid) => {
    if (!cid) return;
    try { localStorage.removeItem(`nextext_draft_${cid}`); } catch { /* ignore */ }
  };
  ...
  // Discard the stashed generated preview (Cancel/Delete): stop playback and
  // revoke the object URL so nothing lingers. Sends nothing.
  const vcDiscardGenerated = () => {
    try { genAudioRef.current?.pause(); } catch {}
    if (genUrl) URL.revokeObjectURL(genUrl);
    setGenBlob(null); setGenUrl(null); setGenDuration(0);
    setGenMime(""); setGenCurrent(0); setGenTotal(0); setGenPlaying(false);
  };

  const vcCloseClone = () => {
    if (vcSending || genSaving) return;
    vcDiscardGenerated();
    vcDiscard();
    setShowClone(false);
  };

  const vcSaveToDevice = async () => {
    if (!genBlob || genSaving) return;
    setGenSaving(true); setVcError("");
    try {
      const voice = availableVoices.find((v) => v.id === vcVoiceId) || availableVoices[0];
      // Real MIME from the generated bytes (NOT a renamed guess).
      const realMime = genBlob.type || genMime || "audio/mpeg";
      // Verify real audio bytes: MP3 frames start with "ID3" or an 0xFF sync.
      // Anything else is refused rather than saved under a wrong extension.
      const head = new Uint8Array(await genBlob.slice(0, 3).arrayBuffer());
      const isMp3 = (head[0] === 0x49 && head[1] === 0x44 && head[2] === 0x33) ||
        (head[0] === 0xff && (head[1] & 0xe0) === 0xe0);
      if (!isMp3) throw new Error("Generated audio didn't verify as playable audio — not saved.");
      const ext = extFromMime(realMime) || "mp3";
      await downloadBlobToDevice(genBlob, `ai-voice-${voice?.id || "note"}.${ext}`, realMime);
    } catch (err) {
      setVcError(err?.message || "Couldn't save to device.");
    }
    setGenSaving(false);
  };
  ...
  const vcConfirmSend = async () => {
    ...
    try {
      const voice = availableVoices.find((v) => v.id === vcVoiceId) || availableVoices[0];
      const mime = genBlob.type || genMime || "audio/mpeg";
      const ext = extFromMime(mime) || "mp3";
      const file = new File([genBlob], `ai-voice-${voice?.id || "note"}.${ext}`, { type: mime });
  ...
  // ── Generated-voice preview player (Play/Pause + seek, replayable) ──
  const fmtGenTime = (secs) => {
    if (!isFinite(secs) || secs <= 0) return "0:00";
    const s = Math.floor(secs);
    return `${Math.floor(s / 60)}:${String(s % 60).padStart(2, "0")}`;
  };
  const toggleGenPlayback = () => {
    const audio = genAudioRef.current;
    if (!audio || !genUrl) return;
    if (genPlaying) {
      audio.pause();
      return;
    }
    try {
      // Replayable: restarting from the top when the previous playthrough ended.
      if (audio.ended || (audio.duration && audio.currentTime >= audio.duration - 0.15)) {
        audio.currentTime = 0;
      }
    } catch {}
    audio.play().catch(() => setVcError("Couldn't play the preview."));
  };
  const seekGenPreview = (val) => {
    const audio = genAudioRef.current;
    const total = genTotal || genDuration || audio?.duration || 0;
    if (!audio || !isFinite(total) || total <= 0) return;
    const clamped = Math.max(0, Math.min(total, Number(val) || 0));
    try { audio.currentTime = clamped; } catch {}
    setGenCurrent(clamped);
  };
            ...
            {showClone && createPortal(
              ...
              <div style={{ position: "fixed", left: "50%", top: "50%", transform: "translate(-50%, -50%)", width: "min(340px, 92vw)", background: t.surface, borderRadius: 16, boxShadow: "0 8px 32px rgba(0,0,0,0.4)", zIndex: 2147481303, padding: 18 }}>
                ...
                {genBlob && genUrl && (
                  <div style={{ padding: "12px 12px", borderRadius: 12, background: t.surface, border: `1px solid ${t.border}`, marginBottom: 10 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: t.text }}>Generated Voice</div>
                      <div onClick={() => { if (!vcSending && !genSaving) vcDiscardGenerated(); }} style={{ fontSize: 12.5, color: t.textMuted, cursor: "pointer", textDecoration: "underline" }}>Delete</div>
                    </div>
                    <audio
                      ref={genAudioRef}
                      src={genUrl}
                      preload="metadata"
                      onLoadedMetadata={() => { const d = genAudioRef.current?.duration; if (isFinite(d) && d > 0) { setGenTotal(d); setGenDuration(d); } }}
                      onTimeUpdate={() => { try { setGenCurrent(genAudioRef.current?.currentTime || 0); } catch {} }}
                      onPlay={() => setGenPlaying(true)}
                      onPause={() => setGenPlaying(false)}
                      onEnded={() => setGenPlaying(false)}
                    />
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div onClick={toggleGenPlayback} style={{ width: 38, height: 38, borderRadius: "50%", background: t.primary, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
                        {genPlaying ? <Pause size={16} color={t.bubbleMeText} /> : <Play size={16} color={t.bubbleMeText} />}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <input
                          type="range"
                          min={0}
                          max={Math.max(genTotal || genDuration || 0, 0.1)}
                          step={0.1}
                          value={Math.min(genCurrent, genTotal || genDuration || 0)}
                          onChange={(e) => seekGenPreview(e.target.value)}
                          style={{ width: "100%", accentColor: t.primary }}
                        />
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: t.textMuted }}>
                          <span>{fmtGenTime(genCurrent)}</span>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
                  <div onClick={vcCloseClone} style={{ flex: 1, textAlign: "center", padding: "11px 0", borderRadius: 10, border: `1px solid ${t.border}`, fontWeight: 700, fontSize: 14, color: t.textMuted, cursor: "pointer" }}>
                    Cancel
                  </div>
                  {!genBlob && (
